...
class FrontController
{
    ...
    private function SimpleRoutes($uri)
    {
    ...
    private function ParamsRoutes($uri)
    {
    ...
    private function ForeachConfigRoute($uri)
    {
        ...
        if (file_exists($realPath) && is_readable($realPath)) {
            ...
            if (method_exists($calledController, $this->_method)) {
                if ($this->isValidRequestMethod($calledController, $this->_method)) {
                    ...
                    throw new \Exception("Method does not allow this method!", 500);
                }
            } else {
                throw new \Exception("Method not found in '" . $file . '.php', 404);
            }
        } else {
    ...
    private function isValidRequestMethod($controller, $method)
    {
        ...
        if (preg_match('/@get/', $comment)) {
            $foundAnnotations[] = 'get';
        }
        if (preg_match('/@post/', $comment)) {
            $foundAnnotations[] = 'post';
        }
        if (preg_match('/@put/', $comment)) {
            $foundAnnotations[] = 'put';
        }
        if (preg_match('/@delete/', $comment)) {
            $foundAnnotations[] = 'delete';
        }

        if (count($foundAnnotations) != 0) {
            if (count($foundAnnotations) > 1) {
                throw new \Exception('Method cannot have more than one method', 500);
            }

            $request = $foundAnnotations[0];
    ...
    private function ValidateAuthorization($doc)
    {
        ...
        if ($matches) {
            if (!SimpleDB::isAdmin()) {
                throw new \Exception("Admin rights!", 401);
            }
        }
        ...
        if ($matches[1]) {
            ...
            if (!SimpleDB::hasRole($role) && !SimpleDB::isAdmin()) {
                $role = ucfirst($role);
                throw new \Exception("$role rithts!", 401);
            }
        }
    }
}